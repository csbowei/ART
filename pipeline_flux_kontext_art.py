...
class FluxKontextPipeline_Art(
    ...
    def prepare_cond_latents(
        ...
        # VAE applies 8x compression; latent dims must also be divisible by 2 for packing.
        ...
        if image is not None:
            ...
            image_latents = self._pack_latents(
                image_latents, batch_size, num_channels_latents, latent_height, latent_width
            )
            image_ids = self._prepare_latent_image_ids(
    ...
    def __call__(
        ...
        max_sequence_length: int = 512,
        # New
        cond: Optional[Union[PipelineImageInput, List[torch.FloatTensor], torch.FloatTensor]] = None,
    ):
        r"""
        ...
        Args:
            ...
            cond (PipelineImageInput, optional): 
                Conditional image for reference.
        ...
        # Preprocess images.
        # Pass the full image (list or single) to preprocess so it handles batches correctly.
        # height/width are forwarded so the image is resized to match the requested generation size.
        if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
            image = self.image_processor.preprocess(image, height=height, width=width)
 
        if cond is not None and not (isinstance(cond, torch.Tensor) and cond.size(1) == self.latent_channels):
            cond = self.image_processor.preprocess(cond, height=height, width=width)
        ...
        cond_latents, cond_latents_ids = self.prepare_cond_latents(
            cond,
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
        ...
        # Concat all position-id sequences: [noisy latents | input image | cond image]
        if image_ids is not None:
            latent_ids = torch.cat([latent_ids, image_ids], dim=0)
        if cond_latents_ids is not None:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Concatenate: [noisy latents | input image latents | cond latents]
